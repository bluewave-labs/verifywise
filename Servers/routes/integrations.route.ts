...
router.post("/test", async (req: Request, res: Response) => {
  try {
    const runtimeConfig = await mlflowService.resolveRuntimeConfig(
      req.tenantId!,
      req.body
    );
...
router.post("/configure", async (req: Request, res: Response) => {
  try {
    ...
    const result = await mlflowService.saveConfiguration(
      req.userId,
      config,
      req.tenantId!
    );
...
router.get("/models", async (req: Request, res: Response) => {
  try {
    ...
    const configSummary = await mlflowService.getConfigurationSummary(
      req.tenantId!
    );
    ...
    const message =
      error instanceof Error ? error.message : "Failed to fetch MLFlow models";
