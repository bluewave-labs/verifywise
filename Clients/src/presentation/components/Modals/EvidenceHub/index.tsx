...
const NewEvidenceHub: FC<NewEvidenceHubProps> = ({
  ...
  const handleTextChange = useCallback(
    (field: keyof EvidenceHubModel) =>
      (event: React.ChangeEvent<HTMLInputElement>) => {
        setValues((prev: EvidenceHubModel) => ({
          ...prev,
          [field]: event.target.value,
        }));
        setErrors((prev: NewEvidenceHubFormErrors) => ({
          ...prev,
          [field]: "",
        }));
      },
  ...
  const handleSelectChange = useCallback(
    (field: keyof EvidenceHubModel) => (event: any) => {
      ...
      setErrors((prev: NewEvidenceHubFormErrors) => ({ ...prev, [field]: "" }));
    },
    []
  );
  ...
  const handleUploadSuccess = (files: FileResponse[]) => {
    ...
    setErrors((prev: NewEvidenceHubFormErrors) => ({ ...prev, files: "" }));
  ...
  const handleRemoveFile = (id: string | number) => {
    setValues((prev: EvidenceHubModel) => ({
      ...prev,
      evidence_files: prev.evidence_files.filter(
        (f: FileResponse) => f.id !== id
      ),
    }));
  };
    ...
    >
      <Stack spacing={6}>
        ...
        {values.evidence_files?.length > 0 && (
          <Stack spacing={2}>
            {values.evidence_files.map((file: FileResponse) => (
              <Box
                key={file.id}
                display="flex"
                alignItems="center"
