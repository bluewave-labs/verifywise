...
export const getLLMKeys = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await logEvent('Error', `Unexpected error fetching LLM Keys: ${(error as Error).message}`);
...
export const getLLMKey = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await logEvent('Error', `Unexpected error fetching LLM Key: ${(error as Error).message}`);
...
export const createLLMKey = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (error instanceof ValidationException) {
      ...
      await logEvent('Error', `Validation error during LLM Key creation: ${error.message}`);
      return res.status(400).json(STATUS_CODE[400](error.message));
    }
    logStructured('error', `unexpected error: ${name}`, functionName, fileName);
    await logEvent('Error', `Unexpected error during LLM Key creation: ${(error as Error).message}`);
...
export const updateLLMKey = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (error instanceof ValidationException) {
      logStructured('error', `validation failed: ${error.message}`, functionName, fileName);
      await logEvent('Error', `Validation error during LLM Key update: ${error.message}`);
      return res.status(400).json(STATUS_CODE[400](error.message));
    }
    logStructured('error', `unexpected error: ${name}`, functionName, fileName);
    await logEvent('Error', `Unexpected error during LLM Key update: ${(error as Error).message}`);
...
export const deleteLLMKey = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await logEvent('Error', `Unexpected error during LLM Key deletion: ${(error as Error).message}`);
