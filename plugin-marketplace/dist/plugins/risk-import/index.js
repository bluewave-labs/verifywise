 ...
 */
async function uninstall(_userId, _tenantId, _context) {
    try {
 ...
 */
async function getExcelTemplate(organizationId, context) {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Risk Import");
    // Fetch users from the organization for dropdown
    let users = [];
    try {
        const usersResult = await context.sequelize.query("SELECT id, name, surname, email FROM public.users WHERE organization_id = :organizationId ORDER BY surname, name", {
    ...
    const userOptions = users.map((user) => `${user.name} ${user.surname} - ${user.email} (ID: ${user.id})`);
    // Add sample data row
    ...
    const addDropdownValidation = (colIndex, values, startRow = 2, endRow = 1000) => {
    ...
    const addDateValidation = (colIndex, startRow = 2, endRow = 1000) => {
        for (let rowNum = startRow; rowNum <= endRow; rowNum++) {
            const cell = worksheet.getCell(rowNum, colIndex);
            cell.dataValidation = {
                type: "date",
    ...
    const addMultiSelectDropdown = (colIndex, values, startRow = 2, endRow = 1000) => {
        for (let rowNum = startRow; rowNum <= endRow; rowNum++) {
            const cell = worksheet.getCell(rowNum, colIndex);
            cell.dataValidation = {
                type: "list",
 ...
 */
function validateRiskRow(row, rowIndex) {
    const errors = [];
    // Required fields
...
function parseRiskRow(row) {
    ...
    // Optional fields
    if (row.ai_lifecycle_phase)
        riskData.ai_lifecycle_phase = row.ai_lifecycle_phase;
    ...
    if (row.impact)
        riskData.impact = row.impact;
    if (row.assessment_mapping)
        riskData.assessment_mapping = row.assessment_mapping;
    if (row.controls_mapping)
        riskData.controls_mapping = row.controls_mapping;
    if (row.likelihood)
        riskData.likelihood = row.likelihood;
    if (row.severity)
        riskData.severity = row.severity;
    // Auto-calculate risk level based on likelihood and severity
    riskData.risk_level_autocalculated = calculateRiskLevel(riskData.likelihood || null, riskData.severity || null);
    if (row.review_notes)
        riskData.review_notes = row.review_notes;
    if (row.mitigation_status)
        riskData.mitigation_status = row.mitigation_status;
    if (row.current_risk_level)
        riskData.current_risk_level = row.current_risk_level;
    if (row.deadline)
        riskData.deadline = new Date(row.deadline);
    if (row.mitigation_plan)
        riskData.mitigation_plan = row.mitigation_plan;
    ...
    if (row.risk_severity)
        riskData.risk_severity = row.risk_severity;
    if (row.final_risk_level)
        riskData.final_risk_level = row.final_risk_level;
    if (row.risk_approval)
        riskData.risk_approval = parseUserId(row.risk_approval);
    if (row.approval_status)
        riskData.approval_status = row.approval_status;
    if (row.date_of_assessment)
 ...
 */
async function importRisks(csvData, tenantId, context) {
    ...
    try {
        ...
        for (let i = 0; i < csvData.length; i++) {
            ...
            try {
                ...
                await sequelize.query(`INSERT INTO "${tenantId}".risks (
