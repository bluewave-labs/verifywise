...
class EventNormalizer {
    ...
    normalize(raw, connectorId) {
        ...
        const tool = (0, ai_tool_registry_1.matchAITool)(url);
        if (!tool) {
     ...
     */
    normalizeBatch(rawEvents, connectorId) {
    ...
    inferActionType(raw) {
        ...
        for (const field of actionFields) {
            ...
            if (value && typeof value === "string") {
                ...
                if (action.includes("upload") || action === "post" || action === "put")
                    return "upload";
                if (action.includes("download") || action === "get")
                    return "download";
                if (action.includes("login") || action.includes("auth"))
                    return "login";
                if (action.includes("api") || action.includes("call"))
                    return "api_call";
                if (action.includes("share"))
                    return "data_share";
                if (action.includes("prompt") || action.includes("chat") || action.includes("completion"))
                    return "prompt";
            }
        }
        ...
        if (url) {
            ...
            if (urlLower.includes("/chat") || urlLower.includes("/completion") || urlLower.includes("/generate"))
                return "prompt";
            if (urlLower.includes("/upload") || urlLower.includes("/files"))
                return "upload";
            if (urlLower.includes("/api/") || urlLower.includes("/v1/") || urlLower.includes("/v2/"))
                return "api_call";
        }
        return "access";
    }
    inferDataClassification(raw) {
        const data = raw.raw_data;
        const classFields = [
        ...
        for (const field of classFields) {
            ...
            if (value && typeof value === "string") {
                ...
                if (cls.includes("restricted") || cls.includes("secret"))
                    return "restricted";
                if (cls.includes("pii") || cls.includes("personal"))
                    return "pii";
                if (cls.includes("phi") || cls.includes("health"))
                    return "phi";
                if (cls.includes("financial") || cls.includes("payment"))
                    return "financial";
                if (cls.includes("confidential"))
                    return "confidential";
                if (cls.includes("internal"))
                    return "internal";
                if (cls.includes("public"))
                    return "public";
            }
        }
        return "unknown";
