...
export const getDistributionSummary = (statusData: StatusData[]): string => {
...
export const getQuickStats = (
  entityType: 'models' | 'vendors' | 'policies' | 'trainings' | 'vendorRisks' | undefined,
  total: number,
  ...
  switch (entityType) {
    case 'models':
      const productionModels = statusData?.find(s => s.label.toLowerCase().includes('production'))?.value || Math.floor(total * 0.4);
      return `${productionModels} in production`;
      
    case 'trainings':
      const completedTrainings = statusData?.find(s => s.label.toLowerCase().includes('completed'))?.value || Math.floor(total * 0.6);
      const completionRate = Math.round((completedTrainings / total) * 100);
      return `${completionRate}% completion rate`;
      
    case 'policies':
      const publishedPolicies = statusData?.find(s => s.label.toLowerCase().includes('published'))?.value || Math.floor(total * 0.5);
      return `${publishedPolicies} published`;
      
    case 'vendors':
      const activeVendors = statusData?.find(s => s.label.toLowerCase().includes('active'))?.value || Math.floor(total * 0.7);
      return `${activeVendors} active`;
      
    case 'vendorRisks':
      const highRisks = statusData?.filter(s => 
        s.label.toLowerCase().includes('high') && !s.label.toLowerCase().includes('very')
      )?.reduce((sum, item) => sum + item.value, 0) || Math.floor(total * 0.2);
      const veryHighRisks = statusData?.find(s => s.label.toLowerCase().includes('very high'))?.value || Math.floor(total * 0.05);
...
export const hasCriticalItems = (
  ...
  switch (entityType) {
    case 'vendorRisks':
      ...
      const veryHighRisk = statusData.find(s => s.label.toLowerCase().includes('very high'))?.value || 0;
    ...
    case 'policies':
      const inReview = statusData.find(s => s.label.toLowerCase().includes('in review'))?.value || 0;
    ...
    case 'trainings':
      const inProgress = statusData.find(s => s.label.toLowerCase().includes('in progress'))?.value || 0;
    ...
    case 'vendors':
      const requiresFollowUp = statusData.find(s => s.label.toLowerCase().includes('follow up'))?.value || 0;
...
export const getPriorityLevel = (
  ...
  switch (entityType) {
    case 'vendorRisks':
      const veryHighRisk = statusData?.find(s => s.label.toLowerCase().includes('very high'))?.value || 0;
      const highRisk = statusData?.find(s => s.label.toLowerCase().includes('high') && !s.label.toLowerCase().includes('very'))?.value || 0;
    ...
    case 'trainings':
      const completed = statusData?.find(s => s.label.toLowerCase().includes('completed'))?.value || 0;
