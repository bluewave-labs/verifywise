...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      ...
      if (!tableExists[0].exists) {
        console.log('Organizations table does not exist yet. Skipping project risk change history table creation.');
      ...
      for (let organization of organizations[0]) {
        ...
        // Check if risks table exists in this tenant schema
        ...
        if (!risksExists[0].exists) {
          console.log(`Skipping tenant ${tenantHash}: risks table does not exist.`);
          continue;
        }
        ...
        // Create index on project_risk_id for faster lookups
        await queryInterface.sequelize.query(`
        ...
        // Create composite index for project_risk_id + changed_at (most common query pattern)
        await queryInterface.sequelize.query(`
  ...
  async down(queryInterface, Sequelize) {
    ...
    try {
      ...
      // If organizations table doesn't exist, skip migration
      if (!tableExists[0].exists) {
        console.log('Organizations table does not exist yet. Skipping project risk change history table rollback.');
        await transaction.commit();
        return;
      }
      ...
      for (let organization of organizations[0]) {
        ...
        await queryInterface.sequelize.query(`
          DROP TABLE IF EXISTS "${tenantHash}".project_risk_change_history;
        `, { transaction });
      }
