...
// Function to ensure tenant infrastructure exists (schema, tables, sequences)
// This should be called before starting the main project creation transaction
export const ensureProjectInfrastructure = async (tenant: string): Promise<void> => {
  ...
  try {
    ...
    await sequelize.query(`
      CREATE TABLE IF NOT EXISTS "${tenant}".projects (
        ...
        uc_id VARCHAR(255) UNIQUE,
        project_title VARCHAR(255),
        owner INTEGER,
        start_date DATE,
        geography INTEGER DEFAULT 1,
    ...
    // 3. Create UC sequence if it doesn't exist
    await sequelize.query(
      `CREATE SEQUENCE IF NOT EXISTS "${tenant}".project_uc_id_seq;`,
      { transaction: setupTransaction }
    );

    // 4. Check if uc_id column exists and add it if needed
    const columnCheck = await sequelize.query(
      `SELECT column_name FROM information_schema.columns
       WHERE table_schema = :tenant AND table_name = 'projects' AND column_name = 'uc_id'`,
      {
        replacements: { tenant },
    ...
    if (columnCheck.length === 0) {
      await sequelize.query(
        `ALTER TABLE "${tenant}".projects ADD COLUMN uc_id VARCHAR(255) UNIQUE;`,
        { transaction: setupTransaction }
      );
    }

    // 5. Populate uc_id for existing projects that don't have one
    const existingProjects = await sequelize.query(
      `SELECT id, uc_id FROM "${tenant}".projects
       WHERE uc_id IS NULL OR uc_id = '' ORDER BY id ASC`,
      {
        type: QueryTypes.SELECT,
        transaction: setupTransaction
      }
    ...
    for (let i = 0; i < existingProjects.length; i++) {
      const project = existingProjects[i] as any;
      const ucId = `UC-${i + 1}`;

      await sequelize.query(
        `UPDATE "${tenant}".projects SET uc_id = :ucId WHERE id = :projectId`,
        {
          replacements: { ucId, projectId: project.id },
          transaction: setupTransaction
        }
      );
    }

    // 6. Set sequence to correct next value
    const totalProjects = await sequelize.query(
      `SELECT COUNT(*) as count FROM "${tenant}".projects`,
      {
        type: QueryTypes.SELECT,
        transaction: setupTransaction
      }
    ...
    const projectCount = (totalProjects[0] as any).count;
    const nextValue = Math.max(1, projectCount);

    await sequelize.query(
      `SELECT setval('"${tenant}".project_uc_id_seq', :nextValue, true);`,
      {
        replacements: { nextValue },
        transaction: setupTransaction
      }
    );

    await setupTransaction.commit();
    console.log(`[Project Utils] Infrastructure setup completed for tenant ${tenant}`);

  } catch (error) {
    await setupTransaction.rollback();
    console.error(`[Project Utils] Infrastructure setup failed for tenant ${tenant}:`, error);
    throw error;
  }
};
