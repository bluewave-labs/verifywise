 ...
 *
 * Usage: node scripts/seedShadowAiDemo.js
 */
...
function generateTimestamp(rng, daysBack) {
  const now = new Date();
...
function buildToolWeights(tools) {
...
async function seedShadowAiDemo() {
  ...
  const rollupKey = (date, email, toolId) => `${date}|${email}|${toolId}`;
