...
export async function getVendorReportMarkdown(
  ...
  const vendorRows: string = vendorData && vendorData.length > 0 ? vendorData.map((vendor) => {
    return `| ${vendor.vendor_name} | ${vendor.assignee} | ${vendor.review_status} | ${vendor.risk_status} | ${vendor.review_date.toLocaleDateString()} |`
  }).join("\n") : "| - | - | - | - | - |";

  const riskRows: string = riskData && riskData.length > 0 ? riskData.map((risk) => {
    return `| ${risk.impact} | ${risk.likelihood} | ${risk.risk_severity} | ${risk.action_owner} | ${risk.risk_level} | ${risk.risk_description} |`
  }).join("\n") : "| - | - | - | - | - | - |";
