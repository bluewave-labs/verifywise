...
export const getAllQuestionsQuery = async (
  ...
  const result = (await sequelize.query(
    ...
    FROM public.questions_struct_eu q JOIN answers_eu a ON q.id = a.question_id WHERE
...
export const getControlStructByControlCategoryIdForAProjectQuery = async (
  ...
  const controlsStruct = (await sequelize.query(
    `SELECT cs.*, c.id AS control_id, c.owner, c.status FROM public.controls_struct_eu cs JOIN controls_eu c ON cs.id = c.control_meta_id
...
export const getControlByIdQuery = async (
  ...
  const result = (await sequelize.query(
    ...
    FROM controls_eu c JOIN public.controls_struct_eu cs ON c.control_meta_id = cs.id WHERE c.organization_id = :organizationId AND c.id = :control_id
...
export const getSubControlsByIdQuery = async (
  ...
  const subControls = (await sequelize.query(
    ...
    FROM subcontrols_eu sc JOIN public.subcontrols_struct_eu scs ON sc.subcontrol_meta_id = scs.id WHERE ${whereClause}
    ORDER BY created_at DESC, id ASC;`,
    {
      replacements,
      ...(transaction && { transaction }),
