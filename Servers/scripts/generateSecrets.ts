...
/**
 * @fileoverview Production Secret Generator
 *
 ...
 * # OR
 * npx ts-node scripts/generateSecrets.ts
 * ```
 *
 * **Output:**
 * - Displays generated secrets for copy-paste into .env
 * - Optionally writes a complete .env template
 * - Provides verification commands for each secret
 *
 * @created 2025-10-06
 */
 ...
 */
function displaySecrets(secrets: ReturnType<typeof generateAllSecrets>) {
...
function calculateEntropy(str: string): number {
  ...
  let entropy = 0;
  for (const freq of Object.keys(charFreq).map(k => charFreq[k])) {
 ...
 */
function generateEnvTemplate(secrets: ReturnType<typeof generateAllSecrets>, outputPath?: string) {
...
export { generateAllSecrets, generateEnvTemplate };