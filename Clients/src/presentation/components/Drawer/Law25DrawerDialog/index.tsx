...
interface Law25DrawerDialogProps {
  open: boolean;
  onClose: (event?: any, reason?: string) => void;
  requirement: any;
  topic: any;
  projectFrameworkId: number | string;
  onSaveSuccess?: (success: boolean, message?: string, requirementId?: number) => void;
}

const Law25DrawerDialog = ({
  open,
  onClose,
  requirement,
  topic,
  ...
  const [fetchedRequirement, setFetchedRequirement] = useState<any>(null);
  ...
  const [selectedRiskForView, setSelectedRiskForView] = useState<any | null>(null);
  const [riskFormData, setRiskFormData] = useState<any>(null);
  const onRiskSubmitRef = useRef<(() => void) | null>(null);

  // Track last fetched requirement ID
  ...
  // Fetch requirement data when drawer opens
  useEffect(() => {
    const fetchRequirementData = async () => {
      ...
      // Skip fetch if we already have data for this requirement
      if (lastFetchedIdRef.current === requirement.id) {
        return;
      }

      setIsLoading(true);
      lastFetchedIdRef.current = requirement.id;

      try {
        const response = await getEntityById({
          routeUrl: `/law-25/requirements/byId/${requirement.id}`,
        });

        if (response.data) {
          setFetchedRequirement(response.data);
          // Update evidence files from fetched data
    ...
    // Reset file states when opening with a new requirement
    if (open && requirement?.id) {
      setUploadFiles([]);
      setDeletedFilesIds([]);
    }
  ...
  // Initialize form data from fetched requirement
  useEffect(() => {
    const reqData = fetchedRequirement || requirement;
    if (reqData) {
      const statusId = statusIdMap.get(reqData.status) || "0";
      setFormData({
        implementation_description: reqData.implementation_description || "",
        status: statusId,
        owner: reqData.owner?.toString() || "",
        reviewer: reqData.reviewer?.toString() || "",
        approver: reqData.approver?.toString() || "",
        auditor_feedback: reqData.auditor_feedback || "",
        risks: reqData.risks || [],
      });

      if (reqData.due_date) {
        setDate(dayjs(reqData.due_date));
      } else {
        setDate(null);
      }
    }
  }, [fetchedRequirement, requirement]);
  ...
  useEffect(() => {
    if (open && fetchedRequirement?.id) {
      fetchLinkedRisks();
    }
  }, [open, fetchedRequirement?.id, formData.risks, selectedRisks, deletedRisks]);
  ...
  const fetchLinkedRisks = async (riskIds?: number[]) => {
    if (!fetchedRequirement?.id) return;
  ...
  const _refreshData = async () => {
    ...
    try {
      const response = await getEntityById({
        routeUrl: `/law-25/requirements/byId/${fetchedRequirement.id}`,
      });

      if (response.data) {
        const reqData = response.data;
        const updatedRiskIds = reqData.risks || [];

        if (reqData.evidence_links) {
          setEvidenceFiles(reqData.evidence_links);
        }
  ...
  const handleSave = async () => {
    if (!fetchedRequirement?.id) {
      handleAlert({
        variant: "error",
        body: "No requirement selected for update",
      });
      return;
    }
    setIsLoading(true);
    try {
      ...
      if (date) formDataToSend.append("due_date", date.format("YYYY-MM-DD"));
      formDataToSend.append("user_id", userId?.toString() || "1");
      formDataToSend.append("deletedFiles", JSON.stringify(deletedFilesIds));
      ...
      const response = await updateEntityById({
        routeUrl: `/law-25/requirements/${fetchedRequirement.id}`,
        body: formDataToSend,
        headers: {
      ...
      if (response.status === 200) {
        // Notify parent of successful save (for row highlighting)
        onSaveSuccess?.(true, "Requirement saved successfully", fetchedRequirement.id);

        // Reset pending state and close the drawer
        resetPendingState();
        lastFetchedIdRef.current = null;
        onClose();
      } else {
        throw new Error(response.data?.message || "Failed to save requirement");
      }
    } catch (error) {
      const errorMessage =
  ...
  const displayData = fetchedRequirement || requirement;

  if (!requirement) {
    return null;
  }

  if (isLoading && !fetchedRequirement) {
    return (
      <Drawer
        open={open}
        onClose={onClose}
      ...
      >
        ...
        >
          <CircularProgress />
          <Typography sx={{ mt: 2 }}>Loading requirement data...</Typography>
        </Stack>
      </Drawer>
    );
  }
  ...
  return (
    <>
      <Drawer
        className="vw-law25-drawer-dialog"
        open={open}
        onClose={onClose}
        sx={{
          width: 600,
      ...
      >
        <Stack
          className="vw-law25-drawer-dialog-content"
          sx={{
            width: 600,
          }}
        >
          ...
          >
            <Typography fontSize={15} fontWeight={700}>
              {displayData?.requirement_id || `${topic?.topic_id}.${requirement?.order_no}`} {displayData?.name}
            </Typography>
            <CloseIcon
              size={20}
              onClick={onClose}
          ...
          <TabContext value={activeTab}>
                ...
                <Box>
                  ...
                  <Stack spacing={2}>
                    <Button
                      variant="contained"
                      onClick={() =>
                        document.getElementById("evidence-file-input-law25")?.click()
                      }
                      disabled={isEditingDisabled}
                      sx={{
                        borderRadius: 2,
            ...
            <TabPanel value="notes" sx={{ padding: "15px 20px" }}>
              <Suspense fallback={<CircularProgress />}>
                {fetchedRequirement?.id && (
                  <NotesTab
                    key={`law25-requirement-${fetchedRequirement.id}`}
                    attachedTo="LAW25_REQUIREMENT"
                    attachedToId={fetchedRequirement.id.toString()}
                  />
                )}
              </Suspense>
            </TabPanel>
          ...
          <Divider />
          <Stack
            className="vw-law25-drawer-dialog-footer"
            sx={{
              display: "flex",
              flexDirection: "row",
...
export default Law25DrawerDialog;
