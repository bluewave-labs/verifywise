...
export async function getVendorReportMarkdown(
  ...
  const vendorRows: string = vendorData && vendorData.length > 0 ? vendorData.map((vendor) => {
    return `| ${vendor.vendor_name} | ${vendor.review_status} |`
  }).join("\n") : "| - | - |";

  const riskRows: string = riskData && riskData.length > 0 ? riskData.map((risk) => {
    return `| ${risk.risk_description} | ${risk.impact} | ${risk.likelihood} | ${risk.risk_severity} |`
  }).join("\n") : "| - | - | - | - |";
...
Vendors table
-------------
| Vendor Name | Review Status |
|----|----|
${vendorRows}

Vendor risk table
-------------
| Risk Description | Impact | Likelihood | Risk Severity |
|----|----|----|----|
${riskRows}
`;
}
