...
const ISO42001Clauses = ({
    ...
    const statusItems = useMemo(
        () => [...statusIdMap.entries()].map(([id, label]) => ({ _id: id, name: label })),
        [statusIdMap]
    );
                    ...
                    filteredSubClauses.map(
                        (
                            ...
                            >
                                ...
                                <Stack>
                                    <Select
                                        disabled={savingId === subClause.id}
                                        sx={{
                                            ...
                                            try {
                                                ...
                                                setSubClausesMap((prev) => {
                                                    ...
                                                    const updated = list.map((sc) =>
                                                        sc.id === subClause.id ? { ...sc, status: newLabel } : sc
                                                    );
                                                    return { ...prev, [clauseIdLocal]: updated };
                                                });


                                                // Sync drawer if open on this subclause
                                                if (drawerOpen && selectedSubClause?.id === subClause.id) {
                                                    setSelectedStatus(newStatusId);
                                                    setSelectedSubClause((prev: any) => (prev ? { ...prev, status: newLabel } : prev));
                                                }
    ...
    useEffect(() => {
        ...
        const localClauseId = selectedSubClause.clause_id; // renamed to avoid shadowing
        const clauseSubClauses = subClausesMap[localClauseId] || [];
        ...
        setSubClausesMap((prev) => ({
            ...prev,
            [localClauseId]: updatedSubClauses,
        }));
    ...
    }, [selectedStatus, drawerOpen, selectedSubClause, subClausesMap, statusIdMap]);
