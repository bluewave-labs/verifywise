...
export const calculateVendorRiskScore = (scorecard: VendorScorecardData): number => {
  ...
  const dataSensitivityValue = scorecard.data_sensitivity 
    ? DATA_SENSITIVITY_VALUES[scorecard.data_sensitivity] / MAX_DATA_SENSITIVITY
    : 0;
    
  const businessCriticalityValue = scorecard.business_criticality
    ? BUSINESS_CRITICALITY_VALUES[scorecard.business_criticality] / MAX_BUSINESS_CRITICALITY  
    : 0;
    
  const pastIssuesValue = scorecard.past_issues
    ? PAST_ISSUES_VALUES[scorecard.past_issues] / MAX_PAST_ISSUES
    : 0;
    
  const regulatoryExposureValue = scorecard.regulatory_exposure
    ? REGULATORY_EXPOSURE_VALUES[scorecard.regulatory_exposure] / MAX_REGULATORY_EXPOSURE
    : 0;
