...
export const getAllClausesWithSubClauseQuery = async (
  ...
  for (let clause of clauses[0]) {
    const subClauses = (await sequelize.query(
      `SELECT sc.id, scs.title, scs.order_no, sc.status, sc.owner FROM public.subclauses_struct_iso scs JOIN subclauses_iso sc ON scs.id = sc.subclause_meta_id WHERE sc.organization_id = :organizationId AND scs.clause_id = :id AND sc.projects_frameworks_id = :projects_frameworks_id ORDER BY scs.id;`,
      {
        replacements: {
          organizationId,
          id: clause.id,
...
export const getSubClausesByClauseIdQuery = async (
  ...
  const subClauses = await sequelize.query(
    ...
    FROM subclauses_iso sc JOIN public.subclauses_struct_iso scs ON
...
export const getSubClauseByIdQuery = async (
  ...
  const subClauses = (await sequelize.query(
    ...
    FROM public.subclauses_struct_iso scs JOIN subclauses_iso sc ON scs.id = sc.subclause_meta_id
...
export const getAllAnnexesWithCategoriesQuery = async (
  ...
  for (let annex of annexes[0]) {
    const annexCategories = (await sequelize.query(
      ...
      FROM public.annexcategories_struct_iso acs JOIN annexcategories_iso ac
...
export const getAnnexCategoriesByIdQuery = async (
  ...
  const annexCategories = (await sequelize.query(
    ...
    FROM public.annexcategories_struct_iso acs JOIN annexcategories_iso ac ON acs.id = ac.annexcategory_meta_id
