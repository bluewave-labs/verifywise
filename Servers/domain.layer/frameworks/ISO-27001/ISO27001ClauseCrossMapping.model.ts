...
import { ISO27001ClauseModel } from "./ISO27001Clause.model";
import { IISO27001ClauseCrossMapping } from "../../interfaces/i.ISO27001ClauseCrossMapping";

@Table({
  tableName: "iso27001clause_cross_mapping",
})
export class ISO27001ClauseCrossMappingModel
  extends Model<ISO27001ClauseCrossMappingModel>
  implements IISO27001ClauseCrossMapping
{
  @Column({
  ...
  @ForeignKey(() => ISO27001ClauseModel)
  @Column({
    type: DataType.INTEGER,
    allowNull: false,
  })
  ...
  /**
   * Create a new ISO27001 Clause Cross Mapping
   */
  static async createNewCrossMapping(
    ...
    relevance: string,
    iso27001clause_id: number
  ): Promise<ISO27001ClauseCrossMappingModel> {
    // Validate clause_no
    ...
    if (!numberValidation(iso27001clause_id, 1)) {
      throw new ValidationException(
    ...
    const crossMapping = new ISO27001ClauseCrossMappingModel();
  ...
  async updateCrossMapping(updateData: {
    ...
    // Validate iso27001clause_id if provided
    if (updateData.iso27001clause_id !== undefined) {
      if (!numberValidation(updateData.iso27001clause_id, 1)) {
        throw new ValidationException(
          "Valid clause ID is required",
          "iso27001clause_id",
          updateData.iso27001clause_id
        );
      }
  ...
  async validateCrossMappingData(): Promise<void> {
    ...
    if (!numberValidation(this.iso27001clause_id, 1)) {
      throw new ValidationException(
        "Valid clause ID is required",
        "iso27001clause_id",
  ...
  /**
   * Check if cross mapping belongs to a specific clause
   */
  ...
  /**
   * Validate uniqueness of clause number within the same parent clause
  ...
  static async validateClauseNumberUniqueness(
    ...
    // const existingMapping = await ISO27001ClauseCrossMappingModel.findOne({
    //   where: { clause_no, iso27001clause_id }
    // });
    // if (existingMapping && existingMapping.id !== excludeMappingId) {
    //   return false; // Clause number already exists for this parent clause
    // }
  ...
  /**
   * Create ISO27001ClauseCrossMappingModel instance from JSON data
   */
  static fromJSON(json: any): ISO27001ClauseCrossMappingModel {
    return new ISO27001ClauseCrossMappingModel(json);
  }
