import { TrainingRegistar, TrainingRegistarModel } from "../models/trainingRegistar.model";
 ...
 */
export const createNewTrainingRegistarQuery = async (trainingRegistar: TrainingRegistar, transaction: Transaction) =>{
    ...
    return Array.isArray(result) && result.length > 0 ? result[0] as TrainingRegistar : null as any;
}
...
export const getAllTrainingRegistarQuery = async(): Promise<TrainingRegistar[]> => {
    ...
    return Array.isArray(trainingRegistars) ? trainingRegistars as TrainingRegistar[] : [];
}

/**
 * Return the training registars by ID // for now keeping it might not need it eventually
 */
export const getTrainingRegistarByIdQuery = async( id: number): Promise<TrainingRegistar> => {
...
export const deleteTrainingRegistarByIdQuery = async(
    ...
    const result = await sequelize.query(
        `DELETE FROM trainingregistar WHERE id = :id`,
        {
            replacements: {id},
            mapToModel: true,
            model:TrainingRegistarModel,
