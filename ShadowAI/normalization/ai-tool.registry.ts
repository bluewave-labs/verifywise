...
import { AIToolCategory, ToolRiskClassification } from "../types/inventory";

export interface AIToolEntry {
  name: string;
  domains: string[];
  category: AIToolCategory;
  default_risk: ToolRiskClassification;
  vendor: string;
  description: string;
}
 ...
 */
export const AI_TOOL_REGISTRY: AIToolEntry[] = [
 ...
 */
export function buildDomainLookup(): Map<string, AIToolEntry> {
  const lookup = new Map<string, AIToolEntry>();
 ...
 */
export function matchAITool(urlOrDomain: string): AIToolEntry | undefined {
 ...
 */
export function getToolsByCategory(category: AIToolCategory): AIToolEntry[] {
  return AI_TOOL_REGISTRY.filter((tool) => tool.category === category);
}

/**
 * Get all high-risk tools.
 */
export function getHighRiskTools(): AIToolEntry[] {
  return AI_TOOL_REGISTRY.filter(
    (tool) => tool.default_risk === "high" || tool.default_risk === "critical"
  );
}
