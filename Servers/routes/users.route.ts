...
async function getUsers(): Promise<User[]> {
  ...
  const usersObj: User[] = usersJSON.map(
    (u: { id: string; name: string; email: string; password: string }) => {
    ...
    return u.map(
      (u) =>
        new UserOut(
          ...
          u.name,
          u.email,
          new Date(u.created_at).toUTCString(),
          new Date(u.last_login).toUTCString()
        )
    );
  } else {
    return new UserOut(
...
router.get("/find", async (req: Request, res: Response) => {
  ...
  if (user === undefined) {
    return res.status(404).json({
      data: `user with ${Object.keys(params)
...
router.post("/", async (req: Request, res: Response) => {
  ...
  for (let u of users) {
    if (u.email === userIn.email) {
      return res
        .status(400)
        .json({ data: `user with email: ${userIn.email} already exists` });
    }
  }
...
router.delete("/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (users.length === filteredUsers.length) {
      return res
        .status(400)
        .json({ data: `user with id: ${userId} not found` });
    } else {
    ...
    if (filteredUser) {
      ...
      if (nonUpdatableFields.length !== 0) {
        return res.status(400).json({
          data: `cannot update ${nonUpdatableFields.join(" and ")} of the user`,
        });
      } else {
        ...
        return res
          .status(200)
          .send({ data: getUserOut(filteredUser) as UserOut });
      }
    } else {
      return res
        .status(400)
...
router.post("/reset-password", async (req: Request, res: Response) => {
  ...
  if (userIndex === -1) {
    return res
      .status(404)
      .json({ data: `user with email: ${email} not found` });
  } else {
    users[userIndex].password = await bcrypt.hash(
      password,
      process.env.SALT as string
    );
    await writeFile("./files/users.json", JSON.stringify(users));
    return res.status(201).send({
      data: `password update successful for the user with email: ${email}`,
    });
  }
});
