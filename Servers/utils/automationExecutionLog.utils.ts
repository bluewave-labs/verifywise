...
export async function createAutomationExecutionLog(
  ...
  const result = await sequelize.query(`INSERT INTO "${tenant}".automation_execution_logs (automation_id, trigger_data, action_results, status, error_message, execution_time_ms, triggered_at, created_at) VALUES (:automation_id, :trigger_data, :action_results, :status, :error_message, :execution_time_ms, NOW(), NOW()) RETURNING *`, {
    replacements: {
      automation_id,
 ...
 */
export async function getAutomationExecutionStats(automation_id: number, tenant: string): Promise<{
